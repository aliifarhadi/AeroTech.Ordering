...
Terminal receipt replay returns the original semantic result and must be distinguishable **internally/testably** from a newly executed mutation through the durable receipt path. A client-visible replay header/flag is not required unless the existing AeroTech presentation framework or the user already defines one; this pack does not invent a replay metadata shape. GET retrieves latest local state. Same key/different payload is a conflict. Pending/Unknown external work must be represented explicitly and must expose the durable operation identity where the surface supports it; it must never masquerade as permanent failure or success. Exact HTTP envelope/header conventions follow the existing framework unless separately versioned/approved.

Error responses must preserve the existing AeroTech framework envelope/code convention unless an explicitly approved VNext API version changes it. Semantically, callers must be able to distinguish malformed input, authentication/authorization failure, missing/undisclosed scope, idempotency/version/claim conflict, known business invalidity/unsupported scenario, pre-dispatch capability unavailability, and an operation whose external effect is Pending/Unknown. The pack does not invent a replacement global error envelope.
