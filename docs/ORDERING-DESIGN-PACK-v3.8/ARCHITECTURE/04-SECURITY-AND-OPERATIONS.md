...
One owning airline per Ordering deployment context. `OwnerAirlineId` must come from a **trusted platform-owned source** and never from arbitrary request input. The current binding is `IHomeOperatorProvider` implemented by `ReferenceDataHomeOperatorProvider`; preserve it unless the user/platform explicitly changes that convention. `ICallerContext` identifies caller/actor scope and is not an alternative source of `OwnerAirlineId`. The pack does not replace it with deployment configuration merely by preference. If the trusted source is ambiguous or conflicting, record a decision blocker instead of inventing one. TenantId, when transported, comes from authenticated platform context, never arbitrary request input. FinancialCustomerId is the responsible customer, not automatically the logged-in user or traveler. Authorization checks both permission and object scope. Agency users cannot read another financial customer's preparations, orders, operations or idempotency results.
