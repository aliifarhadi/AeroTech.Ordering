...
## SC-B0-001 - Domain Messages allowlist

**Given:** Domain.csproj references AeroTech.Messages and ICallerContext uses the current platform identity enum types.

**When:** Run architecture suite against actual Domain type references.

**Then:** Pass only when Domain Messages usage is limited to `AeroTech.Messages.Ordering.Enums.*` plus `BusinessContextType`, `PrincipalType` and `AuthorizationSurface`; fail on integration events, provider/owner wire DTOs, base transport contracts or any other Messages type. Do not remove the project reference merely to make the test pass.
...
**Given:** Empty host and approved SQL Server test persistence exist.
...
**Then:** Same status and Order/result identities; no owner/network access. Replay is proven from the durable receipt/test evidence. Client-visible replay metadata is required only when an existing user-approved AeroTech response convention already defines it; this pack invents no replay header/flag.
