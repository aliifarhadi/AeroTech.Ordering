...
An unresolved live provider must be shown next to LOCAL_DONE, e.g. `LOCAL_DONE; JetPay/coverage LIVE_BLOCKED(BD-005)`. A skipped required local test is not LOCAL_DONE. A stage may continue to simulator-dependent stages after LOCAL_DONE; it cannot claim live completion or production enablement.

## Mandatory stage bundle

Physically generate `artifacts/stages/<stage>/` containing:
