# Pack manifest - version2.0

Target: aliifarhadi/AeroTech.Ordering. Reviewed bootstrap:2f2b9033f675387f68ef7ff95c9d0a575d836fb5. Start with START-HERE and AGENT-START-PROMPT. This replaces the previous FreshBuild instruction set; historical prompts are not competing authorities.

This is a self-contained business specification with explicit owner-binding gaps, not a compiled implementation. Detailed DOMAIN/GOVERNANCE semantics control behavior; SPEC indexes fix names, shapes and traceability. A substantive conflict blocks affected code until reconciled; never silently choose the easier paragraph.
...
MANIFEST.md describes the inventory. SHA256SUMS is added during packaging and covers every other file, including the final validation report. Companion ZIP, consolidated Markdown and separate Agent prompt are delivered outside this root.

## Requirement counts

18 stages;60 invariants;149 acceptance scenarios;31 canonical command/query contracts;10 semantic ports;17 event types;52 expanded interactions. These are requirements, not executed runtime tests.

## Structural validation

```text
python -m pip install -r tools/requirements-validation.txt
python tools/validate_pack.py . --checksums
```

Runtime scripts described in RUNBOOKS must be delivered by the implementation Agent in the target repository; this specification does not pretend to contain a working airline service.
